"use client";

import { useSelectionBounds } from "@/hooks/use-selection-bounds";
import { Camera, Color } from "@/types/canvas";
import { memo } from "react";
import { useSelf } from "@/liveblocks.config";
import { ColorPicker } from "./color-picker";
import { useMutation } from "@liveblocks/react";
import { useDeleteLayer } from "@/hooks/use-delete-layers";
import { Button } from "@/components/ui/button";
import { Hint } from "@/components/hint";
import { BringToFront,SendToBack, Trash2 } from "lucide-react";
import { LiveList, LiveMap, LiveObject } from "@liveblocks/client";
import { Layer } from "@/types/canvas";


interface SelectionToolsProps {
    camera: Camera;
    setLastUsedColor: (color: Color) => void;
    isStorageLoaded: boolean;
};

export const SelectionTools = memo(({
    camera,
    setLastUsedColor,
    isStorageLoaded,

}: SelectionToolsProps) => {

    const selection = useSelf((me) => me.presence.selection) as string[] | undefined;

    const moveToback = useMutation((
        {storage}
    ) => {
        const liveLayers = storage.get("layers") as LiveMap<string, LiveObject<Layer>> | undefined;
        const liveLayerIds = storage.get("layerIds") as LiveList<string> | undefined;
        if (!liveLayerIds || !selection) return;
        const indices: number[] = [];

        const arr = liveLayerIds.toArray();

        for (let i = 0; i < arr.length; i++) {
            if (selection.includes(arr[i])) {
                indices.push(i);
            }
        }
        for (let i = 0; i < indices.length; i++) {
            liveLayerIds.move(indices[i], i);
        }
    }, [selection])


    const moveToFront = useMutation((
        {storage}
    ) => {
        const liveLayers = storage.get("layers") as LiveMap<string, LiveObject<Layer>> | undefined;
        const liveLayerIds = storage.get("layerIds") as LiveList<string> | undefined;
        if (!liveLayerIds) return;
        const indices: number[] = [];

        const arr = liveLayerIds.toArray();

        for (let i = 0; i < arr.length; i++) {
            if (selection?.includes(arr[i])) {
                indices.push(i);
            }
        }
        for (let i = indices.length - 1; i >= 0; i--) {
            liveLayerIds.move(indices[i], arr.length - 1 - (indices.length - 1 - i));
        }

    }, [selection])


    const setFill = useMutation((
        { storage },
        fill: Color
    ) => {
        if (!isStorageLoaded) return;

        const liveLayers = storage.get("layers") as LiveMap<string, LiveObject<Layer>> | undefined;
        setLastUsedColor(fill);

        if (liveLayers && selection) {
            selection.forEach((id: string) => {
                liveLayers.get(id)?.set("fill", fill);
            });
        }
    }, [selection, setLastUsedColor, isStorageLoaded]);


    const deleteLayer = useDeleteLayer();


    const selectionBounds = useSelectionBounds();

    if (!selectionBounds){
        return null;
    }

    const x = selectionBounds.width / 2 + selectionBounds.x + camera.x;
    const y = selectionBounds.y + camera.y; 

    return (
        <div className="absolute p-3 rounded-xl bg-white shadow-sm border flex selection-none"
        style = {{
            transform: `translate(
                calc(${x}px - 50%),
                calc(${y - 16}px - 100%)
            )`
        }}
        >
            <ColorPicker
                onChange = {setFill}
            />

            <div className="flex flex-col gap-y-0.5">
                <Hint label = "Bring to front">
                    <Button
                        variant = "board"
                        size = "icon"
                        onClick={moveToFront}
                    >
                        <BringToFront />
                    </Button>
                </Hint>
                <Hint label = "Send to back" side = "bottom">
                    <Button
                        variant = "board"
                        size = "icon"
                        onClick={moveToback}
                    >
                        <SendToBack />
                    </Button>
                </Hint>
            </div>

            <div className="flex items-center pl-2 ml-2 border-l border-neural-200">
                <Hint label="Delete">
                    <Button
                        variant = "board"
                        size = "icon"
                        onClick={deleteLayer}
                    >
                        <Trash2/>
                    </Button>
                </Hint>
            </div>
        </div>
    );
});

SelectionTools.displayName = "SelectionTools";
